...
export default async function getCategoryProducts(context, params) {
  if (params.input.type && params.input.type === 'instant-search') {
    const url = new URL(context.config.api.url + context.config.api.restPath + '/productSearch');
    ...
    const url = new URL(context.config.api.url + context.config.api.restPath + '/categoryProducts');
