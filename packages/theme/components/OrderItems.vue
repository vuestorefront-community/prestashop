...
export default {
  ...
  setup(props) {
    const { orders: details, search: detailsSearch } = useUserOrder();

    const orderDetails = ref(null);
    onBeforeMount(async ()=>{
      await detailsSearch({orderId: props.order.id});
      orderDetails.value = orderGetters.getOrderDetailsFiltered(details.value);
    });
    return {
      orderGetters,
      orderProducts: computed(() => orderDetails.value ? orderDetails.value.products : [])
    };
  }
};
</script>
